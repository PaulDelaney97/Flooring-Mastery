...
// method readBigDecimal and readLocalDate in this file.
...
public class UserIOConsoleImpl implements UserIO {
    ...
    public LocalDate readLocalDate(String prompt) {
        ...
        while (invalidInput) {
            try {
                System.out.println(prompt);
                // Get date from user as a string and try parse to a local date
                String inputDate = console.nextLine();
                date = LocalDate.parse(inputDate); // will break if wrong format
                invalidInput = false;
            } catch (DateTimeException e) {
                this.print("Input Error: Date is not in te correct format (YYYY-MM-DD)");
            }
        }
        return date;
    }
